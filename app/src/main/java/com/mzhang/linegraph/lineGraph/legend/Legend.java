...
import com.mzhang.linegraph.lineGraph.chartDefaults.ComponentBase;
...
public class Legend extends ComponentBase {
    ...
    public Legend() {
        ...
        this.mTextSize = ChartUtils.convertDpToPixel(10f);
        this.mXOffset = ChartUtils.convertDpToPixel(5f);
        this.mYOffset = ChartUtils.convertDpToPixel(3f); // 2
    }
    ...
    public float getMaximumEntryWidth(Paint p) {
        ...
        float formToTextSpace = ChartUtils.convertDpToPixel(mFormToTextSpace);
        ...
        for (LegendEntry entry : mEntries) {
            ...
            float length = (float) ChartUtils.calcTextWidth(p, label);
    ...
    public float getMaximumEntryHeight(Paint p) {
        ...
        for (LegendEntry entry : mEntries) {
            ...
            float length = (float) ChartUtils.calcTextHeight(p, label);
    ...
    public void setExtra(int[] colors, String[] labels) {
        ...
        for (int i = 0; i < Math.min(colors.length, labels.length); i++) {
            ...
            else if (entry.formColor == ChartUtils.COLOR_NONE)
     ...
     * @param labelpaint
     */
    public void calculateDimensions(Paint labelpaint, ChartDimens chartDimens) {

        float defaultFormSize = ChartUtils.convertDpToPixel(mFormSize);
        float stackSpace = ChartUtils.convertDpToPixel(mStackSpace);
        float formToTextSpace = ChartUtils.convertDpToPixel(mFormToTextSpace);
        float xEntrySpace = ChartUtils.convertDpToPixel(mXEntrySpace);
        float yEntrySpace = ChartUtils.convertDpToPixel(mYEntrySpace);
        ...
        switch (mOrientation) {
            case VERTICAL: {
                ...
                float labelLineHeight = ChartUtils.getLineHeight(labelpaint);
                ...
                for (int i = 0; i < entryCount; i++) {
                    ...
                    float formSize = Float.isNaN(e.formSize)
                            ? defaultFormSize
                            : ChartUtils.convertDpToPixel(e.formSize);
                    ...
                    if (label != null) {
                        ...
                        width += ChartUtils.calcTextWidth(labelpaint, label);
            ...
            case HORIZONTAL: {
                ...
                float labelLineHeight = ChartUtils.getLineHeight(labelpaint);
                float labelLineSpacing = ChartUtils.getLineSpacing(labelpaint) + yEntrySpace;
                float contentWidth = chartDimens.contentWidth() * mMaxSizePercent;
                ...
                for (int i = 0; i < entryCount; i++) {
                    ...
                    if (label != null) {
                        ...
                        mCalculatedLabelSizes.add(ChartUtils.calcTextSize(labelpaint, label));
