package com.mzhang.linegraph.lineGraph.legend;
...
import com.mzhang.linegraph.lineGraph.chartRenderers.BaseRenderer;
...
public class LegendRenderer extends BaseRenderer {
    ...
    public LegendRenderer(ChartDimens chartDimens, Legend legend) {
        super(chartDimens);

        this.mLegend = legend;

        mLegendLabelPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mLegendLabelPaint.setTextSize(ChartUtils.convertDpToPixel(9f));
    ...
    public void computeLegend(ChartData<?> data) {
        ...
        mLegend.calculateDimensions(mLegendLabelPaint, mChartDimens);
    }
    ...
    public void renderLegend(Canvas c) {
        ...
        float labelLineHeight = ChartUtils.getLineHeight(mLegendLabelPaint, legendFontMetrics);
        float labelLineSpacing = ChartUtils.getLineSpacing(mLegendLabelPaint, legendFontMetrics)
                + ChartUtils.convertDpToPixel(mLegend.getYEntrySpace());
        float formYOffset = labelLineHeight - ChartUtils.calcTextHeight(mLegendLabelPaint, "ABC") / 2.f;

        LegendEntry[] entries = mLegend.getEntries();

        float formToTextSpace = ChartUtils.convertDpToPixel(mLegend.getFormToTextSpace());
        float xEntrySpace = ChartUtils.convertDpToPixel(mLegend.getXEntrySpace());
        ...
        float defaultFormSize = ChartUtils.convertDpToPixel(mLegend.getFormSize());

        // space between the entries
        float stackSpace = ChartUtils.convertDpToPixel(mLegend.getStackSpace());
        ...
        switch (horizontalAlignment) {
            case LEFT:
                ...
                if (orientation == Legend.LegendOrientation.VERTICAL)
                    ...
                    originPosX = mChartDimens.contentLeft() + xoffset;
            ...
            case RIGHT:
                ...
                if (orientation == Legend.LegendOrientation.VERTICAL)
                    originPosX = mChartDimens.getChartWidth() - xoffset;
                else
                    originPosX = mChartDimens.contentRight() - xoffset;
            ...
            case CENTER:
                ...
                if (orientation == Legend.LegendOrientation.VERTICAL)
                    originPosX = mChartDimens.getChartWidth() / 2.f;
                else
                    originPosX = mChartDimens.contentLeft()
                            + mChartDimens.contentWidth() / 2.f;
        ...
        switch (orientation) {
            case HORIZONTAL: {
                ...
                switch (verticalAlignment) {
                    ...
                    case BOTTOM:
                        posY = mChartDimens.getChartHeight() - yoffset - mLegend.mNeededHeight;
                        break;

                    case CENTER:
                        posY = (mChartDimens.getChartHeight() - mLegend.mNeededHeight) / 2.f + yoffset;
                        break;
                }
                ...
                for (int i = 0, count = entries.length; i < count; i++) {
                    ...
                    float formSize = Float.isNaN(e.formSize) ? defaultFormSize : ChartUtils.convertDpToPixel(e.formSize);
            ...
            case VERTICAL: {
                ...
                switch (verticalAlignment) {
                    case TOP:
                        posY = (horizontalAlignment == Legend.LegendHorizontalAlignment.CENTER
                                ? 0.f
                                : mChartDimens.contentTop());
                        posY += yoffset;
                        break;

                    case BOTTOM:
                        posY = (horizontalAlignment == Legend.LegendHorizontalAlignment.CENTER
                                ? mChartDimens.getChartHeight()
                                : mChartDimens.contentBottom());
                        posY -= mLegend.mNeededHeight + yoffset;
                        break;

                    case CENTER:
                        posY = mChartDimens.getChartHeight() / 2.f
                ...
                for (int i = 0; i < entries.length; i++) {
                    ...
                    if (e.label != null) {
                        ...
                        if (direction == Legend.LegendDirection.RIGHT_TO_LEFT)
                            posX -= ChartUtils.calcTextWidth(mLegendLabelPaint, e.label);
    ...
    protected void drawForm(
        ...
        if (entry.formColor == ChartUtils.COLOR_SKIP ||
                entry.formColor == ChartUtils.COLOR_NONE ||
                entry.formColor == 0)
            return;
        ...
        final float formSize = ChartUtils.convertDpToPixel(
        ...
        switch (form) {
            ...
            case LINE:
            {
                final float formLineWidth = ChartUtils.convertDpToPixel(
